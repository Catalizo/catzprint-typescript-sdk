...
class ApiClient {
    ...
    async sendRequest(endpoint, method = 'POST', data) {
        ...
        try {
            ...
            throw new errors_js_1.ApiConnectionException(`Network error: ${message}`);
        }
        finally {
            clearTimeout(timer);
        }
        ...
        if (!response.ok) {
            ...
            throw new errors_js_1.PrintingException(`API Error ${response.status}: ${String(detail).slice(0, 200)}`);
        }
        return body;
    }
}
